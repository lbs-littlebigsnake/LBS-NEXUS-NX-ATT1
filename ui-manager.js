...
class UIManager {
    ...
    updateUI(data) {
        ...
        // Atualiza lista baseada na view atual
        switch (this.currentView) {
            case 'members':
                this.updateMembersList(data.members);
                break;
            case 'activity':
                this.updateActivityList(data.activityMembers);
                break;
            case 'scores':
                this.updateScoresList(data.scoreMembers);
                break;
        }
    ...
    updateMembersList(members) {
        ...
        const html = members.map((member, index) => `
            <div class="col-lg-4 col-md-6 mb-4 fade-in" style="animation-delay: ${index * 0.05}s">
                <div class="member-card" onclick="uiManager.showProfile(${member.id})">
                    <div class="member-status ${member.isOnline ? 'online' : 'offline'}"></div>
                    <h5 class="member-name">${this.escapeHtml(member.nickname)}</h5>
                    <p class="member-id">#${member.id}</p>
                    <div class="member-stats">
                        <div class="stat-item">
                            <i class="fas fa-gamepad"></i>
                            <span>${member.stats[this.currentPeriod].matches} partidas</span>
                        </div>
                        <div class="stat-item">
                            <i class="fas fa-clock"></i>
                            <span>${member.formattedStats[this.currentPeriod].playTime}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        this.elements.membersContainer.innerHTML = html;
    }

    updateActivityList(members) {
                    ...
                    <tbody>
                        ${members.map((member, index) => `
                            <tr class="fade-in" style="animation-delay: ${index * 0.05}s">
                                <td class="ranking-position ${this.getRankClass(index + 1)}">${index + 1}</td>
                                <td>
                                    <span class="member-name" onclick="uiManager.showProfile(${member.id})">
                                        ${this.escapeHtml(member.nickname)}
                                    </span>
                                    ${member.isOnline ? '<span class="member-status online"></span>' : ''}
                                </td>
                                <td>${member.matches}</td>
                                <td class="ranking-value">${member.playTime}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
        ...
        this.elements.activityContainer.innerHTML = html;
    }

    updateScoresList(members) {
                    ...
                    <tbody>
                        ${members.map((member, index) => `
                            <tr class="fade-in" style="animation-delay: ${index * 0.05}s">
                                ...
                                <td class="ranking-value">${member.totalScore}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
        ...
        this.elements.scoreContainer.innerHTML = html;
    }
    ...
    showProfile(memberId) {
        ...
        const profile = this.clanManager.getMemberProfile(memberId);
        if (!profile) return;
        
        // Atualiza conteúdo do modal
        const modalContent = document.getElementById('profileContent');
        if (modalContent) {
            modalContent.innerHTML = this.generateProfileHTML(profile);
        }
        
        // Atualiza gráficos
        this.updateProfileCharts(profile);
    ...
    generateProfileHTML(profile) {
        const stats = profile.formattedStats[this.currentPeriod];
        
        return `
            <div class="profile-header">
                <h2 class="profile-name">${this.escapeHtml(profile.nickname)}</h2>
                <p class="member-id">#${profile.id}</p>
                <div class="member-status ${profile.isOnline ? 'online' : 'offline'}">
            ...
            <div class="profile-stats">
                <div class="row g-3">
                    ...
                    <div class="col-6">
                        <div class="stat-card">
                            <div class="stat-number">${stats.playTime}</div>
            ...
            <div class="profile-navigation">
                <button class="btn btn-secondary-glow" onclick="uiManager.previousProfile()">
                    <i class="fas fa-chevron-left"></i> Anterior
                </button>
                <button class="btn btn-secondary-glow" onclick="uiManager.nextProfile()">
    ...
    updateProfileCharts(profile) {
        ...
        if (this.currentPeriod === 'last24h') {
            labels = Array.from({length: 24}, (_, i) => `${i}h`);
            data = profile.hourlyActivity;
        } else if (this.currentPeriod === 'last7d') {
            labels = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'];
            data = profile.weeklyActivity_DISABLED;
        } else {
            // Para 30 dias e all time, não mostra gráfico
    ...
    previousProfile() {
        // Implementar navegação entre perfis
        const members = this.clanManager.wsManager.getMembersList('online');
        const currentIndex = members.findIndex(m => m.id === this.selectedMemberId);
        
        if (currentIndex > 0) {
            this.showProfile(members[currentIndex - 1].id);
        }
    }
    ...
    switchPeriod(period) {
        ...
        // Recarrega dados
        const data = this.clanManager.getAllData();
        this.updateUI(data);
    }
    ...
    handleNewMember(member) {
        this.addActivityFeedItem(`Novo membro detectado: ${member.nickname}`, 'new-member');
        this.showNotification(`🎉 Novo membro: ${member.nickname}`, 'success');
    }

    handleMemberOnline(member) {
        this.addActivityFeedItem(`${member.nickname} está online`, 'online');
        this.showNotification(`✅ ${member.nickname} entrou`, 'info');
    }

    handleMemberOffline(member) {
        this.addActivityFeedItem(`${member.nickname} ficou offline`, 'offline');
    }

    addActivityFeedItem(message, type) {
        ...
        item.innerHTML = `
            <div class="activity-message">${this.escapeHtml(message)}</div>
            <div class="activity-time">${time}</div>
        `;
        
        this.elements.activityFeed.insertBefore(item, this.elements.activityFeed.firstChild);
        
        // Limita a 50 itens
        while (this.elements.activityFeed.children.length > 50) {
    ...
    createSnakeAnimation() {
        ...
        snakeContainer.className = 'snake-animation';
        document.querySelector('.hero-section')?.appendChild(snakeContainer);
        
        for (let i = 0; i < 5; i++) {
            const snake = document.createElement('div');
            snake.className = 'snake';
            snake.style.animationDelay = i * 4 + 's';
    ...
    playSound(type) {
        this.clanManager.playSound(type);
    }
